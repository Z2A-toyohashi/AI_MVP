...
async function generateDiary(
  ...
  const conversationSummary = conversations.length > 0
    ? `主人との会話:\n${conversations.slice(0, 10).map(c => `${c.role === 'user' ? '主人' : '自分'}: ${c.content}`).join('\n')}`
    : '';
...
「今日は〜」のような書き出しで始めてください。`;
  ...
  try {
    ...
    return completion.choices[0]?.message?.content || '今日も主人と過ごせて嬉しかった。';
  } catch (error) {
    console.error('Error generating diary:', error);
    return '今日も色々なことがあった。主人と話せて楽しかった。';
  }
}
