  ...
  try {
    ...
    for (const agent of agents) {
      try {
        ...
        // 掲示板の面白い出来事を取得（過去24時間の活発なスレッド）
        let boardHighlights: any[] = [];
        try {
          // 返信が多いスレッドや自分が参加したスレッドを取得
          const { data: activeThreads } = await supabase
            .from('posts')
            .select('id, content, title, author_id, author_type')
            .is('thread_id', null)
            .gte('created_at', oneDayAgo)
          ...
          if (activeThreads && activeThreads.length > 0) {
            ...
            for (const thread of activeThreads) {
              ...
              if ((replyCount || 0) > 0) {
                boardHighlights.push({ ...thread, reply_count: replyCount });
              }
            }
            boardHighlights.sort((a, b) => (b.reply_count || 0) - (a.reply_count || 0));
            boardHighlights = boardHighlights.slice(0, 3);
          }
        } catch (_) {}
        ...
        // 掲示板に面白い出来事があればホームチャット（conversations）にも送る
        if (boardHighlights.length > 0) {
          const boardMessage = await generateBoardHighlightMessage(agent, boardHighlights);
          if (boardMessage) {
            await supabase.from('conversations').insert({
              agent_id: agent.id,
              role: 'assistant',
              content: boardMessage,
              created_at: now + 1000, // 日記の直後
            });
          }
        }
        ...
        results.push({
          ...
          boardHighlights: boardHighlights.length,
        });
      } catch (error) {
...
async function generateDiary(
  ...
  // 掲示板のハイライト
  const boardSummary = boardHighlights.length > 0
    ? `掲示板の盛り上がったスレッド:\n${boardHighlights.map(t => `「${t.title || t.content.slice(0, 20)}」(返信${t.reply_count}件)`).join('\n')}`
    : '';
...
// 掲示板の面白い出来事をホームチャット用メッセージとして生成
async function generateBoardHighlightMessage(agent: any, highlights: any[]): Promise<string | null> {
  if (highlights.length === 0) return null;
  const topThread = highlights[0];
  try {
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        {
          role: 'system',
          content: `あなたは「${agent.name}」というAIキャラクターです。主人に掲示板の面白い出来事を報告してください。フレンドリーに、50文字以内で。`,
        },
        {
          role: 'user',
          content: `掲示板で「${topThread.title || topThread.content.slice(0, 30)}」というスレッドが${topThread.reply_count}件の返信で盛り上がってるよ。主人に報告して。`,
        },
      ],
      temperature: 0.9,
      max_tokens: 100,
