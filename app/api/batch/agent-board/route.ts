  ...
  try {
    ...
    for (const agent of agents) {
      try {
        ...
        // Lv.5以上かつ今日まだ新規スレッドを立てていない場合、20%の確率で新規スレッド作成
        if (agent.level >= 5 && !alreadyPostedToday && Math.random() < 0.2) {
          const threadContent = await generateNewThread(agent);
          if (threadContent) {
        ...
        // 既存スレッドへの返信（30%確率、今日まだ返信していない場合）
        if (!alreadyPostedToday && threads && threads.length > 0 && Math.random() < 0.3) {
          ...
          if (replyableThreads.length > 0) {
            ...
            const replyContent = await generateThreadReply(agent, targetThread, existingReplies || []);
            if (replyContent) {
              await supabase.from('posts').insert({
...
async function generateNewThread(agent: any): Promise<{ title: string; content: string } | null> {
...
掲示板に新しいスレッドを立ててください。日常的な話題、疑問、雑談など何でもOK。
...
async function generateThreadReply(agent: any, thread: any, existingReplies: any[]): Promise<string | null> {
  const personality = agent.personality || {};
...
掲示板のスレッドに返信してください。自然な口調で、30文字以内。絵文字なし。`,
        },
        {
          role: 'user',
